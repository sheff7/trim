...
import 'GifPreview.dart';

class GifView extends StatefulWidget {
  final String file;

  const GifView({Key? key, required this.file}) : super(key: key);
...
// default, so you do not need to use this.
        ...
        .saveTrimmedVideo(
            ...
            onSave: (String? outputPath) {
              ...
              Navigator.of(context).pushReplacement(
                MaterialPageRoute(
                  builder: (context) => GifPreviw(outputVideoPath: outputPath.toString(),),
                ),
              );
              print('Gifff' + outputPath.toString());
            })
        .then((value) {
      setState(() {
        // _value = value;
  ...
  void _loadVideo() {
    _trimmer.loadVideo(videoFile: File(widget.file.toString()));
  }
                      ...
                      : () async {
                          _saveVideo().then((outputPath) {
                            ...
                            final snackBar = SnackBar(
                                content: Text('Gif Saved successfully'));
